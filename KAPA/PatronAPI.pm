...
sub find_patron {
	...
	my $request_uri = "http://$server/PATRONAPI/$barcode/dump";

	my $request = HTTP::Request->new(GET => "http://$server/PATRONAPI/$barcode/dump");	
	my $result = $userAgent->request($request);
	my $retrieved_patron = 0;
	if( $result->is_success){
		my $result_string = $result->as_string;
		
		if($result_string =~ m/P BARCODE/)
		{
