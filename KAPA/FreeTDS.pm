...
sub find_patron {
	...
	$db_connection = DBI->connect("DBI:Sybase:server=$server", $username, $password, {RaiseError => 1});
	$db_connection->do("use $database_name");

	@retrieved_patron = $db_connection->selectrow_array("SELECT bbarcode AS barcode, borrower.pin# AS pin FROM borrower_barcode JOIN borrower ON (borrower.borrower# = borrower_barcode.borrower#) WHERE bbarcode = '$barcode'");
	...
	print STDERR "BC: ", $retrieved_patron[0], "PIN: ", $retrieved_patron[1];
	
	if(scalar @retrieved_patron ne 0) {
		return KAPA::Patron->new($retrieved_patron[0], $retrieved_patron[1]);
	}
	return undef;
